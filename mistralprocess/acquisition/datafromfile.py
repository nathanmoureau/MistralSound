...
def fromJaz(filePath, n_lines):
    ...
    with open(filePath) as f:
        for count,x in enumerate(f):
            ...
            lambda1[count], intensity1[count], lambda2[count], intensity2[count], lambda3[count], intensity3[count] = brute_getFloat(x) 
...
def brute_getFloat(string):
    ...
    for i,c in enumerate(string):
        if c == '\t':
            ...
            count += 1
    # print(sep)
    return float(string[0:sep[0]]), float(string[sep[0]+1:sep[1]]), float(string[sep[1]+1:sep[2]]), float(string[sep[2]+1:sep[3]]), float(string[sep[3]+1:sep[4]]), float(string[sep[4]+1:-1]) 
...
def writeWavFromCsv(filepath, fileout = "out.wav", delimiter = ",", skip_header = 5, rate = 48000):
